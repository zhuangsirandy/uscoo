...
import { bindings, db, HttpError, now, trustedUserId, uid } from './case-store';
...
export async function createAssessmentLead(req: Request, input: LeadInput) {
  ...
  if (input.requestId) {
    const existing = await db()
      .prepare('SELECT id FROM assessment_leads WHERE id=? AND visitor_id=?')
      .bind(input.requestId, input.visitorId)
      .first<{ id: string }>();
    if (existing)
      return {
        id: existing.id,
        emailStatus: await dispatchMail(`assessment:${existing.id}`).catch(
          () => 'unknown' as const,
        ),
      };
  }
  ...
  const recent = await db()
    .prepare(
      'SELECT COUNT(*) AS n FROM assessment_leads WHERE (visitor_id=? OR email=?) AND created>=?',
    )
    .bind(
      input.visitorId,
      input.email,
  ...
  if (Number(recent?.n || 0) >= 5)
    throw new HttpError(429, '今天已经保存过多次，请稍后再试。');
  ...
  await db().batch([
    ...
    mailStatement({
      ...
      subject:
        input.language === 'en'
          ? 'Your USCOO assessment and next steps'
          : '你的 USCOO 初步评估与下一步准备清单',
      text: assessmentReport(input),
    }),
  ]);
...
export async function createSupportRequest(req: Request, input: SupportInput) {
  ...
  if (input.requestId) {
    const existing = await db()
      .prepare('SELECT id FROM support_requests WHERE id=? AND visitor_id=?')
      .bind(input.requestId, input.visitorId)
      .first<{ id: string }>();
    if (existing)
      return {
        ...
        email,
        emailStatus: await dispatchMail(`support-notify:${existing.id}`).catch(
          () => 'unknown' as const,
        ),
      };
  }
  ...
  await db().batch([
    ...
    mailStatement({
      ...
      to: 'coo@uscoo.ai',
      replyTo: email,
      subject: `USCOO · New request ${id.slice(0, 8)}`,
...
export async function getAdminDashboard() {
  const [
    ...
    db()
      ...
      .first<{ total: number }>(),
    db()
      .prepare('SELECT COUNT(*) AS total FROM assessment_leads')
      .first<{ total: number }>(),
    db()
      .prepare(
    ...
    db()
      .prepare(
        'SELECT id,name,email,wechat,location,timing,company,application_path,has_chatgpt,wants_interview,language,created FROM assessment_leads ORDER BY created DESC LIMIT 100',
      )
      .all(),
    db()
      .prepare(
    ...
    db()
      .prepare(
        'SELECT id,kind,reference_id,recipient,status,attempts,error,created FROM email_outbox ORDER BY created DESC LIMIT 100',
      )
      .all(),
  ]);
  return {
...
export async function updateSupportRequest(
  ...
  const item = await db()
    .prepare('SELECT email,language FROM support_requests WHERE id=?')
    .bind(input.id)
    .first<{ email: string; language: string }>();
  if (!item) throw new HttpError(404, '未找到这条问题。');
  if (input.status === 'answered' && !input.reply.trim())
    throw new HttpError(422, '请先填写回复。');
  const update = db()
    .prepare(
      'UPDATE support_requests SET status=?,reply=?,updated=? WHERE id=?',
    )
    .bind(input.status, input.reply, now(), input.id);
  ...
  await db().batch([
    ...
    mailStatement({
      ...
      to: item.email,
      subject: `USCOO · ${item.language === 'en' ? 'Reply to your request' : '你的咨询回复'} ${input.id.slice(0, 8)}`,
      text: input.reply,
    }),
  ]);
  return {
    emailStatus: await dispatchMail(key).catch(() => 'unknown' as const),
  };
}
...
export const assessmentLeadInputSchema = z
  .object({
    requestId: z.string().uuid().optional(),
    visitorId: z.string().uuid(),
    name: z.string().trim().max(200).default(''),
...
export async function sendStoredAssessment(id: string) {
  const item = await db()
    .prepare('SELECT * FROM assessment_leads WHERE id=?')
    .bind(id)
    .first<any>();
  if (!item) throw new HttpError(404, '未找到评估记录。');
  const input = {
    language: item.language === 'zh' ? ('zh' as const) : ('en' as const),
    location: item.location,
    timing: item.timing,
    company: item.company,
    applicationPath: item.application_path,
